...
\node (l1) at (4,7.5) [right]{\tt observations <- [...]  };
\node (l2) at (4,6.5) [right]{\tt alpha <- 2.0  };
\node (l3) at (4,6) [right]{\tt beta <- 4.0  };
\node (l4) at (4,5.5) [right]{\tt M $\sim$ dnGamma(alpha, beta)  };
\node (l5) at (4,4.5) [right]{\tt lambda <- 1.0  };
\node (l6) at (4,4) [right]{\tt sigma $\sim$ dnExponential(lambda)  };
\node (l7) at (4,3) [right]{\tt mu :$=$ ln(M) - ((sigma * sigma) / 2.0) };
\node (l8) at (4,2.25) [right]{\tt N <- observations.size() };
\node (l8) at (4,1.75) [right]{\tt for( i in 1:N )\{ };
\node (l9) at (4.4,1.25) [right]{\tt x[i] $\sim$ dnLognormal(mu, sigma) };
\node (l10) at (4.4,0.75) [right]{\tt x[i].clamp(observations[i]) };
\node (l11) at (4,0.25) [right]{\tt \} };
\end{tikzpicture}
