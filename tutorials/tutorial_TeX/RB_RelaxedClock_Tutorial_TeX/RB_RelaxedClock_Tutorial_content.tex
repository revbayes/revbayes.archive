...
{\tt \begin{snugshade*}
\begin{lstlisting}
root_time ~ dnLdnNorm(mu_ra, stdv_ra, offset=tHesperocyon)
\end{lstlisting}
\end{snugshade*}}
...
sdv <- 0.25
mu <- ln(M) - ((sdv * sdv) * 0.5)
crown_Ursid_fossil ~ dnLdnNorm(mu, sdv, offset=n_TMRCA_Ursidae)
\end{lstlisting}
\end{snugshade*}}
...
base_rate ~ dnLdnNorm(br_mu, br_s)
...
for(i in 1:n_branches){
   branch_rates[i] ~ dnLdnNorm(ucln_mu, ucln_sigma)
