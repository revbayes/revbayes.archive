...
\end{eqnarray*}
Alternatively, we can formalize the idea that we are totally ignorant about the \emph{scale} of $\sigma$. In other words, we could use a log-uniform prior:
\begin{eqnarray*}
...
Finally, the tree topology $\tau$ is, as mentioned above, fixed to some externally given phylogeny.
...
You may find it convenient to program this multivariate model by first copying the script of the univariate model (\cmd{placentaliaMass.Rev}).
...
\\
\cmd{nu $\sim$ \text{Exponential}(0.001)}
\item
