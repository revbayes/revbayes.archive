...
define $\sigma$:
\\
\cmd{sigma $\sim$ \text{Exponential}(0.001)}
\\
...
% this move applies a random translation across the entire phylogeny:
\\
\cmd{
...
define $\kappa$:
\\
\cmd{kappa $\sim$ \text{Exponential}(0.001)}
\item
...
In the following, we outline some possible extensions or variations, based on the same modeling philosophy.
...
\subsection*{Using comparative data about fossils}

\subsection*{Systematic trends: Cope's rule}

One account for a possible systematic trend in body size evolution (i.e. Cope's rule) by using a Brownian motion with drift. Note, however, that systematic trends cannot be estimated using only extant taxa (at least with a simple comparative model such as the one constructed above). If you have fossil data, on the other hand, you can estimate a trend. To do this, you should use a time-calibrated phylogeny with fossils (as tips sampled in the past) and include estimated fossil body masses in the matrix of quantitative traits. Then, in addition to what has been described above, you would just need to define a drift parameter, with a diffuse prior centered on 0:
\\
\cmd{
copestrend $\sim$ norm(0,10)
}
\\
and create a Brownian motion with drift:
\\
\cmd{logmass $\sim$ brownian(tau,sigma,drift=copestrend)}
\\
Of course, you would need to move the trend parameter (using a regular sliding move). After running the model, you could visualize the posterior distribution on this trend parameter and assess the empirical support in favor of Cope's rule by estimating the posterior probability that this trend parameter is positive.
