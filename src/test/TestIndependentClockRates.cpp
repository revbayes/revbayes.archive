...
bool TestIndependentClockRates::run( void ) {
	...
	for( size_t i=0; i<numBranches; i++){
		moves.push_back( new ScaleMove(bratesNotConst[i], log(2.0), true, 2.0) );
	}
	...
	std::string logFN = "clock_test/test_RB_gmc_IGR_23May_1_pr.log";
	monitors.push_back( new FileMonitor( monitoredNodes, 10, logFN, "\t" ) );
	
    std::set<DagNode*> monitoredNodes2;
    monitoredNodes2.insert( tau );
	
	std::string treFN = "clock_test/test_RB_gmc_IGR_23May_1_pr.tre";
