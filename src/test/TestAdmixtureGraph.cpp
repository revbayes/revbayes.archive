...
//    seed.push_back(12); seed.push_back(2); GLOBAL_RNG->setSeed(seed);
    ...
    SnpData* snps = PopulationDataReader().readSnpData2(fn,1);
...
//    numProcesses=80;
    ...
    if (updateParameters)
    {
        moves.push_back( new ScaleMove(diffusionRate, 0.5, false, 10.0) );
        moves.push_back( new ScaleMove(diversificationRate, 1.0, false, 10.0) );
        moves.push_back( new ScaleMove(turnover, 1.0, false, 10.0) );
    }
    ...
    if (updateTopology)
    {
        moves.push_back( new AdmixtureNarrowExchange( tau, 0.5, numTaxa) );
        moves.push_back( new AdmixtureSubtreePruneRegraft( tau, 0.5, numTaxa) );
    ...
    if (updateNodeAges)
    {
        for (size_t i = numTaxa; i < numNodes - 1; i++)
            moves.push_back( new AdmixtureNodeTimeSlideBeta( tau, (int)i, 0.5, false, 2.0 ) );
    }
    ...
    if (useBranchRates)
    {
        ...
        for( int i=0; i < numBranches; i++)
            moves.push_back( new ScaleMove(branchRates_nonConst[i], 0.5, false, 2.0) );
        
        // tree rate shift
        moves.push_back( new AdmixtureShiftTreeRates(diffusionRate, branchRates_nonConst, 0.5, false, 10));
        
        // shift node age for branch rate
        for (size_t i = numTaxa; i < numNodes - 1; i++)
        {
            if (updateNodeAges)
                moves.push_back( new AdmixtureShiftNodeAgeAndRate(tau, branchRates_nonConst, (int)i, 0.5, false, 2.0) );
    ...
    if (useAdmixtureEdges)
    {
        ...
        moves.push_back( new AdmixtureEdgeReplaceResidualWeights( tau, admixtureRate, branchRates_nonConst, residuals, delay, allowSisterAdmixture, 20.0) );
        ...
        if (updateTopology)
            ;//moves.push_back( new AdmixtureEdgeDivergenceMerge( tau, admixtureRate, branchRates_nonConst, admixtureCount, residuals, delay, allowSisterAdmixture, 10.0 ));

        moves.push_back( new AdmixtureEdgeReweight( tau, delay, 10.0, 10.0) );
        moves.push_back( new AdmixtureEdgeReversePolarity( tau, delay, 10.0, 10.0) );
        moves.push_back( new AdmixtureEdgeSlide( tau, branchRates_nonConst, delay, allowSisterAdmixture, 10.0, 10.0) );
        moves.push_back( new ScaleMove(admixtureRate, 0.5, false, 5.0));
