...
TestGtrGammaModel::TestGtrGammaModel(const std::string &afn, const std::string &tFn, int gen) : alignmentFilename( afn ), treeFilename( tFn ), mcmcGenerations( gen ){
    
}

TestGtrGammaModel::~TestGtrGammaModel() {
    // nothing to do
}
...
//    moves.push_back( new SimplexMove( pi, 100.0, 4, 0, true, 2.0 ) );
    ...
    moves.push_back( new MetropolisHastingsMove( new ScaleProposal(alpha, 1.0), true, 1.0 ) );
