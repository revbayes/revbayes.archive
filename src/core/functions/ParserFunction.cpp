...
/** Basic constructor. */
ParserFunction::ParserFunction(InferenceFunction *f, const std::string &fn, const ArgumentRules &ar, const TypeSpec &rt, bool t ) : RbFunction(), function( f ), functionName( fn), argRules( ar ), returnType( rt ), throwsError( t ), args( ) {
    
    argsProcessed = false;
}

/** Copy constructor. */
ParserFunction::ParserFunction(const ParserFunction &x) : RbFunction(x), args(  ), function( x.function->clone() ), functionName( x.functionName ), argRules( x.argRules ), throwsError( x.throwsError ), returnType( x.returnType ) {
 ...
 */
bool  ParserFunction::checkArguments( const std::vector<Argument>& passedArgs, std::vector<unsigned int>* matchScore) {
...
void ParserFunction::clear(void) {
...
/* Delete processed args */
void ParserFunction::clearArguments(void) {
...
int ParserFunction::computeMatchScore(const DAGNode *arg, const ArgumentRule &rule) {
 ...
 */
const RbLanguageObject& ParserFunction::executeFunction(void) {
...
const std::vector<Argument>& ParserFunction::getArguments(void) const {
    return args;
}


std::vector<Argument>& ParserFunction::getArguments(void) {
    return args;
}
 ...
 *  6. If there are still empty slots, the arguments do not match the rules.
 */
void ParserFunction::processArguments( const std::vector<Argument>& passedArgs ) {
...
void ParserFunction::setArgument(const std::string& name, const Argument& arg) {
