...
//
///** Execute function */
//RbPtr<RbLanguageObject> Func_normalizeVector::executeFunction(const std::vector<const RbObject *> &args) {
//
//    // Get first element
//    const RlVector& tempVec    = static_cast<const RlVector& >( *args[0] );
//    double          desiredSum = static_cast<const RealPos&  >( *args[1] ).getValue();
//    
//    // type conversion
...
//    RlVector *normalizedVector = new RlVector( Probability::getClassTypeSpec() );
//    
...
//
//    if (!rulesSet) {
//        argumentRules.push_back( new ConstArgumentRule( "vector", TypeSpec( RlVector::getClassTypeSpec(), new TypeSpec(RealPos::getClassTypeSpec() ) ) ) );
...
//    
//    static TypeSpec returnTypeSpec = TypeSpec( RlVector::getClassTypeSpec(), new TypeSpec( Probability::getClassTypeSpec() ) );
//    
