...
/** Execute function */
RbPtr<RbLanguageObject> Func_dppConFromExpNumClusters::executeFunction(const std::vector<const RbObject *> &args) {
    
	double clust  = static_cast<const RealPos&>( *args[0] ).getValue();
	double num    = static_cast<const RealPos&>( *args[1] ).getValue();
