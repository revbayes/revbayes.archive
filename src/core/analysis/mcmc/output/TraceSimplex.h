#ifndef TraceSimplex_H
#define TraceSimplex_H
...
#define NOT_CHECKED 2
    ...
    class TraceSimplex : public Trace {
        
    public:
        
        TraceSimplex(void);
        
        virtual                     ~TraceSimplex(void) {}
        
        bool                        operator==(const TraceSimplex &t) const                { return this == &t; }
        bool                        operator!=(const TraceSimplex &t) const                { return !this->operator==( t ); }
        bool                        operator<(const TraceSimplex &t) const                 { return this < &t; }
        
        // overloaded functions from RbObject
        TraceSimplex*               clone(void) const;                                              //!< Clone object
        
        void                        addValueFromString(const std::string &s);
        void                        addObject(Simplex d);
    ...
    std::ostream&                       operator<<(std::ostream& o, const TraceSimplex& x);                                //!< Overloaded output operator
