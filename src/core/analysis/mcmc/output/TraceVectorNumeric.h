#ifndef TraceVectorNumeric_H
#define TraceVectorNumeric_H
...
#define NOT_CHECKED 2
    ...
    class TraceVectorNumeric : public Trace {
        
    public:
        
        TraceVectorNumeric(void);
        
        virtual                     ~TraceVectorNumeric(void) {}
        
        bool                        operator==(const TraceVectorNumeric &t) const                { return this == &t; }
        bool                        operator!=(const TraceVectorNumeric &t) const                { return !this->operator==( t ); }
        bool                        operator<(const TraceVectorNumeric &t) const                 { return this < &t; }
        
        // overloaded functions from RbObject
        TraceVectorNumeric*         clone(void) const;                                              //!< Clone object
        
        void                        addValueFromString(const std::string &s);
        void                        addObject(RbVector<double> d);
...
//        void                        setValues(std::vector<double> v)                { values = v; }
    ...
    std::ostream&                       operator<<(std::ostream& o, const TraceVectorNumeric& x);                                //!< Overloaded output operator
