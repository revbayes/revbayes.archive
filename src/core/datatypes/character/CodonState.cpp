...
unsigned int CodonState::computeState(const std::string &symbol) const
{
    ...
    throw RbException("Cannot construct a codon from a single character.");
}
...
AminoAcidState CodonState::getAminoAcidState( void ) const
{
    ...
    else if ( (codon_pos_2 & 0x400) == 0x400 ) // test if the first codon position is a 'C'
    {
...
bool CodonState::isAmbiguous( void ) const
{
