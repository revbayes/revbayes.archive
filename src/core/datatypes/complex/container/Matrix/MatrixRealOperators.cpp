 ...
 * @return A - b
 */
Matrix<Real> operator-(const Matrix<Real>& A, const double& b) {
 ...
 * @return a + B
 */
Matrix<Real> operator+(const double& a, const Matrix<Real>& B) {
...
Matrix<Real> operator/(const Matrix<Real>& A, const Matrix<Real>& B) {
	...
	Matrix<Real> invB(N, N, new Real( 0.0 ) );
    RbMath::matrixInverse(B, invB);
    
	Matrix<Real> C(N, N, new Real( 0.0 ) );
 ...
 * @return A += b
 */
Matrix<Real> &operator+=(Matrix<Real>& A, const double& b) {
...
Matrix<Real> operator+(const Matrix<Real>& A, const Matrix<Real>& B) {
    ...
    {
		Matrix<Real> C(m, n, new Real( 0.0 ) );
...
Matrix<Real> operator*(const Matrix<Real>& A, const Matrix<Real>& B) {
	...
	Matrix<Real> C(M, K, new Real( 0.0 ) );
	for (size_t i=0; i<M; i++) {
 ...
 */
Matrix<Real>&  operator+=(Matrix<Real>& A, const Matrix<Real>& B) {
