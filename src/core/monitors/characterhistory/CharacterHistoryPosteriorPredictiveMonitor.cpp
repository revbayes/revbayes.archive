...
/* Constructor */
CharacterHistoryPosteriorPredictiveMonitor::CharacterHistoryPosteriorPredictiveMonitor(TypedDagNode<TimeTree>* t,  std::vector<StochasticNode<BranchHistory>* > bh, int g, const std::string &fname, const std::string &del, bool pp, bool l, bool pr, bool ap, bool sm, bool sr) : Monitor(g,t), outStream(), tree( t ), branchHistories(bh), filename( fname ), separator( del ), posterior( pp ), likelihood( l ), prior( pr ), append(ap), showMetadata(sm), showRates(sr) {
...
void CharacterHistoryPosteriorPredictiveMonitor::monitor(long gen) {
