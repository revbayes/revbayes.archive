...
/* Constructor */
AdmixtureBipartitionMonitor::AdmixtureBipartitionMonitor(TypedDagNode<AdmixtureTree> *t,  TypedDagNode< std::vector< double > >* br, TypedDagNode<int>* dt, int ntr, int nar, int g, const std::string &fname, const std::string &del, bool pp, bool l, bool pr) : Monitor(g,t), outStream(), tree( t ), branchRates(br), delayTimer(dt), filename( fname ), separator( del ), posterior( pp ), likelihood( l ), prior( pr ), numSamples(0), numTreeResults(ntr), numAdmixtureResults(nar), numTaxa(0) {
...
void AdmixtureBipartitionMonitor::updateBipartitions(void)
{
    ...
    if (delayTimer->getValue() <= 0)
    {
        ...
        for (size_t i = 0; i < nodes.size(); i++)
        {
            ...
            else if (&p->getAdmixtureChild() != NULL)
            {
                ...
                // if key exists, push on to existing value as copy of original node
                if (admixtureBipartitions.find(p_key) != admixtureBipartitions.end() && admixtureBipartitions.at(p_key).find(c_key) != admixtureBipartitions.at(p_key).end())
                {
                    admixtureBipartitions[p_key][c_key].push_back(er);
                }
                ...
                {
                    std::vector<AdmixtureEdgeRecord> tmp(1,er);
                    admixtureBipartitions[p_key][c_key] = tmp;
                }
            }
        }
        numSamples++;
...
std::string AdmixtureBipartitionMonitor::buildTreeBipartitionString(void)
{
...
std::string AdmixtureBipartitionMonitor::buildAdmixtureBipartitionString(void)
{
    if (delayTimer->getValue() > 0) return "";
    
    // insert all map elements into set sorted by container sizes in decreasing order
    std::multiset< std::pair< std::pair< std::vector<bool>, std::vector<bool> >, std::vector<AdmixtureEdgeRecord> >, AdmixtureBipartitionCompare> s;
    ...
    for (it1 = admixtureBipartitions.begin(); it1 != admixtureBipartitions.end(); it1++)
    {
    ...
    size_t curResults = 0;
    std::multiset< std::pair< std::pair< std::vector<bool>, std::vector<bool> >, std::vector<AdmixtureEdgeRecord> >, AdmixtureBipartitionCompare>::iterator its;
    ...
    for (size_t i = 0; i < bestPairs.size(); i++)
    {
        ...
        std::vector<AdmixtureEdgeRecord>* v = &(admixtureBipartitions[bestPairs[i].first][bestPairs[i].second]);
        ss << "\tpp: " << std::setprecision(8) << std::fixed << (double)v->size() / numSamples << "\tn:" << std::setw(8) << v->size() << "\tN: " << std::setw(8) << numSamples;
...
void AdmixtureBipartitionMonitor::monitor(long gen) {
    ...
    if (gen % samplingFrequency == 0) {
        ...
        std::string ap = buildAdmixtureBipartitionString();
        std::cout << "\n" << ap;
