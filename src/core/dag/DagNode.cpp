...
DagNode::DagNode( const DagNode &n ) : Parallelizable( n ),
    ...
    visit_reinit_count ( n.visit_reinit_count ),
    ref_count( 0 )
{
...
DagNode& DagNode::operator=(const DagNode &d)
{
    ...
    if ( &d != this )
    {
        ...
        visit_reinit_count = d.visit_reinit_count;
    }
...
void DagNode::reInitializeAffected( void )
{
    ...
    for ( std::vector<DagNode *>::iterator i = children.begin(); i != children.end(); i++ )
    {
        ...
        // visit child only if it has not yet been visited
        if (child->visit_reinit_count == 0) {
            child->reInitializeMe();
        }

        // increment visit count for child
        child->visit_reinit_count += 1;

        // clear visit count for child after visited by all parents
        if ( child->visit_reinit_count >= child->getParents().size() ) {
            child->visit_reinit_count = 0;
        }
