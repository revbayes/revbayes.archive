...
double RbStatistics::F::pdf(double m, double n, double x, bool give_log)
{
    ...
    if (x == 0.) return(m > 2 ? 0.0 : (m == 2 ? 1.0 : RbConstants::Double::inf));
    
    if (!RbMath::isFinite(m) && !RbMath::isFinite(n)) { /* both +Inf */
        if(x == 1.) return RbConstants::Double::inf;
        /* else */  return 0.0;
    }
...
//    R_P_bounds_01(x, 0., ML_POSINF);
    ...
    if (df1 * x > df2)
        x = RbStatistics::Beta::cdf(df2 / (df2 + df1 * x), df2 / 2., df1 / 2.);
    else
        x = RbStatistics::Beta::cdf(df1 * x / (df2 + df1 * x), df1 / 2., df2 / 2.);
...
double RbStatistics::F::quantile(double df1, double df2, double p)
{
    ...
    p = (1. / RbStatistics::Beta::quantile(p, df2/2, df1/2) - 1.) * (df2 / df1);
    return p;
}
...
{
    double v1, v2;
    if (RbMath::isNan(n1) || RbMath::isNan(n2) || n1 <= 0. || n2 <= 0.) { 
        std::ostringstream s;
        s << "Cannot draw random value for F distribution with df1 = " << n1 << " and df2 = " << n2;
        throw (RbException(s));
    }    
    
    v1 = RbMath::isFinite(n1) ? (RbStatistics::ChiSquare::rv(n1,rng) / n1) : 1;
    v2 = RbMath::isFinite(n2) ? (RbStatistics::ChiSquare::rv(n2,rng) / n2) : 1;
    return v1 / v2;
}
