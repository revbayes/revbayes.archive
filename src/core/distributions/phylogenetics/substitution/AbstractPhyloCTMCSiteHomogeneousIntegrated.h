#ifndef AbstractPhyloCTMCSiteHomogeneousIntegrated_H
#define AbstractPhyloCTMCSiteHomogeneousIntegrated_H
...
namespace RevBayesCore {
     ...
     *                    nodeIndex*num_site_rates*pattern_block_size*num_chars +
     ...
     * site_rate_offset               =  pattern_block_size*num_chars;
     * siteOffset                  =  num_chars;
     * This gives the more convenient access via
     * partialLikelihoods[active*activeLikelihoodOffset + nodeIndex*nodeOffset + siteRateIndex*site_rate_offset + siteIndex*siteOffset + charIndex]
     *
    ...
    class AbstractPhyloCTMCSiteHomogeneousIntegrated : public TypedDistribution< AbstractHomologousDiscreteCharacterData >, public TreeChangeEventListener {
        
    public:
        // Note, we need the size of the alignment in the constructor to correctly simulate an initial state
        AbstractPhyloCTMCSiteHomogeneousIntegrated(const TypedDagNode<Tree> *t, size_t nChars, size_t nMix, bool c, size_t nSites, bool amb );
        AbstractPhyloCTMCSiteHomogeneousIntegrated(const AbstractPhyloCTMCSiteHomogeneousIntegrated &n);                                                                                          //!< Copy constructor
        virtual                                                            ~AbstractPhyloCTMCSiteHomogeneousIntegrated(void);                                                              //!< Virtual destructor
        
        // public member functions
        // pure virtual
        virtual AbstractPhyloCTMCSiteHomogeneousIntegrated*                           clone(void) const = 0;                                                                      //!< Create an independent clone
    ...
    protected:
        ...
        virtual void                                                        updateTransitionProbabilities(size_t nodeIdx, double brlen, size_t siteRate, size_t matrix_index, size_t clock_index);
        ...
        TransitionProbabilityMatrix                                         transition_prob_matrix;
        ...
        bool                                                                integrate_over_subst_matrices;
        bool                                                                integrate_over_clock_rates;
...
template<class charType>
RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::AbstractPhyloCTMCSiteHomogeneousIntegrated(const TypedDagNode<Tree> *t, size_t nChars, size_t nMix, bool c, size_t nSites,  bool amb) :
...
num_site_rates( nMix ),
tau( t ),
transition_prob_matrix( TransitionProbabilityMatrix(num_chars) ),
...
pattern_block_size( num_patterns )
{
    ...
    homogeneousRateMatrix       = new ConstantNode<RateGenerator>("rateMatrix", new RateMatrix_JC( num_chars ) );
    ...
    integrate_over_subst_matrices               = false;
    integrate_over_clock_rates                  = false;
...
template<class charType>
RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::AbstractPhyloCTMCSiteHomogeneousIntegrated(const AbstractPhyloCTMCSiteHomogeneousIntegrated &n) :
...
tau( n.tau ),
transition_prob_matrix( n.transition_prob_matrix ),
//    partialLikelihoods( new double[2*num_nodes*num_site_rates*num_sites*num_chars] ),
partialLikelihoods( NULL ),
...
pattern_block_size( n.pattern_block_size )
{
    ...
    integrate_over_subst_matrices               = n.integrate_over_subst_matrices;
    integrate_over_clock_rates                  = n.integrate_over_clock_rates;
 ...
 */
template<class charType>
RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::~AbstractPhyloCTMCSiteHomogeneousIntegrated( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::bootstrap( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::compress( void )
{
    ...
    std::vector<size_t> indexOfsite_pattern;
    ...
    if ( compressed )
    {
        ...
        for (size_t site = 0; site < num_sites; ++site)
        {
            ...
            else
            {
                ...
                indexOfsite_pattern.push_back( site );
    ...
    for (std::vector<TopologyNode*>::iterator it = nodes.begin(); it != nodes.end(); ++it)
    {
        if ( (*it)->isTip() )
        {
            ...
            // resize the column
            char_matrix[nodeIndex].resize(pattern_block_size);
            gap_matrix[nodeIndex].resize(pattern_block_size);
            for (size_t patternIndex = 0; patternIndex < pattern_block_size; ++patternIndex)
            {
                ...
                charType &c = static_cast<charType &>( taxon.getCharacter(siteIndices[indexOfsite_pattern[patternIndex+pattern_block_start]]) );
                gap_matrix[nodeIndex][patternIndex] = c.isGapState();
                
                if ( usingAmbiguousCharacters == true )
                {
                    // we use the actual state
                    char_matrix[nodeIndex][patternIndex] = c.getState();
                }
                else
                {
                    // we use the index of the state
                    char_matrix[nodeIndex][patternIndex] = c.getStateIndex();
                }
    ...
    for (size_t i=0; i<pattern_block_size; ++i)
    {
        ...
        {
            if ( c != char_matrix[j][i] || gap_matrix[j][i] == true )
            {
                inv = false;
                break;
            }
...
template<class charType>
double RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::computeLnProbability( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::computeMarginalNodeLikelihood( size_t nodeIndex, size_t parentNodeIndex )
{
    
    // compute the transition probability matrix
    this->updateTransitionProbabilities( nodeIndex, this->tau->getValue().getNode(nodeIndex).getBranchLength(), 0, 0, 0 );
    ...
    for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
    {
        ...
        p_mixture+=this->site_rate_offset; p_mixture_marginal+=this->site_rate_offset; p_parent_mixture_marginal+=this->site_rate_offset;
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::computeMarginalRootLikelihood( void )
{
    // get the root node
    const TopologyNode &root = tau->getValue().getRoot();
    ...
    for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
    {
        ...
        // increment the pointers to the next mixture category
        p_mixture+=this->site_rate_offset; p_mixture_marginal+=this->site_rate_offset;
 ...
 */
template<class charType>
std::vector<charType> RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::drawAncestralStatesForNode(const TopologyNode &node)
{
 ...
 */
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::drawJointConditionalAncestralStates(std::vector<std::vector<charType> >& startStates, std::vector<std::vector<charType> >& endStates)
{
    ...
    for (size_t i = 0; i < this->num_sites; i++)
    {
        ...
        for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
        {
            ...
            p_site       += this->site_rate_offset;
            p_left_site  += this->site_rate_offset;
            p_right_site += this->site_rate_offset;
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::recursivelyDrawJointConditionalAncestralStates(const TopologyNode &node, std::vector<std::vector<charType> >& startStates, std::vector<std::vector<charType> >& endStates, const std::vector<size_t>& sampledSiteRates)
{
    RandomNumberGenerator* rng = GLOBAL_RNG;
    ...
    this->updateTransitionProbabilities( nodeIndex, node.getBranchLength(), 0, 0, 0 );
    ...
    for (size_t i = 0; i < this->num_sites; i++)
    {
        ...
        // get ptr to first mixture cat for site
        //        p_site          = p_node  + cat * this->site_rate_offset + pattern * this->siteOffset;
        const double* p_left_site_mixture_j     = p_left  + cat * this->site_rate_offset + pattern * this->siteOffset;
        const double* p_right_site_mixture_j    = p_right + cat * this->site_rate_offset + pattern * this->siteOffset;
        
        // iterate over possible end states for each site given start state
        for (size_t j = 0; j < this->num_chars; j++)
        {
            double tp_kj = this->transition_prob_matrix[k][j];
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::tipDrawJointConditionalAncestralStates(const TopologyNode &node, std::vector<std::vector<charType> >& startStates, std::vector<std::vector<charType> >& endStates, const std::vector<size_t>& sampledSiteRates)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::fillLikelihoodVector(const TopologyNode &node, size_t nodeIndex)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::fireTreeChangeEvent( const RevBayesCore::TopologyNode &n )
{
...
template<class charType>
std::vector<size_t> RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::getIncludedSiteIndices( void )
{
    // create a vector with the correct site indices
    // some of the sites may have been excluded
...
template<class charType>
std::vector<double> RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::getRootFrequencies( void ) const
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::keepSpecialization( DagNode* affecter )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::recursivelyFlagNodeDirty( const RevBayesCore::TopologyNode &n ) {
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::recursiveMarginalLikelihoodComputation( size_t nodeIndex )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::redrawValue( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::reInitialized( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::resizeLikelihoodVectors( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::restoreSpecialization( DagNode* affecter )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::scale( size_t nodeIndex)
{
    ...
    if ( use_scaling == true && nodeIndex % RbSettings::userSettings().getScalingDensity() == 0 )
    {
        ...
        for (size_t site = 0; site < this->pattern_block_size ; ++site)
        {
            ...
            for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
            {
                ...
                size_t offset = mixture*this->site_rate_offset + site*this->siteOffset;
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::scale( size_t nodeIndex, size_t left, size_t right )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::scale( size_t nodeIndex, size_t left, size_t right, size_t middle )
{
 ...
 */
template <class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setActivePIDSpecialized(size_t a, size_t n)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setValue(AbstractHomologousDiscreteCharacterData *v, bool force)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::simulate( const TopologyNode &node, std::vector< DiscreteTaxonData< charType > > &taxa, const std::vector<bool> &invariant, const std::vector<size_t> &perSiteRates)
{
    ...
    for (std::vector< TopologyNode* >::const_iterator it = children.begin(); it != children.end(); ++it)
    {
        ...
        updateTransitionProbabilities( child.getIndex(), child.getBranchLength(), 0, 0, 0 );
        ...
        for ( size_t i = 0; i < num_sites; ++i )
        {
            ...
            else
            {
                ...
                double *freqs = transition_prob_matrix[ parentState ];
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setClockRate(const TypedDagNode< double > *r)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setClockRate(const TypedDagNode< RbVector< double > > *r)
{
    ...
    integrate_over_clock_rates = r->getValue().size() != (num_nodes-1);
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setMcmcMode(bool tf)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setPInv(const TypedDagNode< double > *r)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setRateMatrix(const TypedDagNode< RateGenerator > *rm) {
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setRateMatrix(const TypedDagNode< RbVector< RateGenerator > > *rm) {
    ...
    integrate_over_subst_matrices = rm->getValue().size() != (num_nodes-1);
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setRootFrequencies(const TypedDagNode< RbVector< double > > *f)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setSiteRates(const TypedDagNode< RbVector< double > > *r)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::setUseMarginalLikelihoods(bool tf)
{
...
template<class charType>
std::vector< std::vector<double> >* RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::sumMarginalLikelihoods( size_t nodeIndex )
{
    ...
    for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
    {
        ...
        // increment the pointers to the next mixture category
        p_mixture_marginal+=this->site_rate_offset;
...
template<class charType>
double RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::sumRootLikelihood( void )
{
    // get the root node
    const TopologyNode &root = tau->getValue().getRoot();
    ...
    for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
    {
        ...
        // increment the pointers to the next mixture category
        p_mixture+=this->site_rate_offset;
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::swapParameterInternal(const DagNode *oldP, const DagNode *newP)
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::touchSpecialization( DagNode* affecter, bool touchAll )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::updateMarginalNodeLikelihoods( void )
{
...
template<class charType>
void RevBayesCore::AbstractPhyloCTMCSiteHomogeneousIntegrated<charType>::updateTransitionProbabilities(size_t nodeIdx, double brlen, size_t site_rate_index, size_t matrix_index, size_t clock_rate_index)
{
    ...
    {
        if ( integrate_over_subst_matrices == true )
        {
            rm = &this->heterogeneousRateMatrices->getValue()[matrix_index];
        }
        else
        {
    ...
    {
        if ( integrate_over_clock_rates == true )
        {
            rate = this->heterogeneous_clock_rates->getValue()[clock_rate_index];
        }
        else
        {
    ...
    if ( this->rateVariationAcrossSites == true )
    {
        ...
        rm->calculateTransitionProbabilities( startAge, endAge,  rate * r[site_rate_index], this->transition_prob_matrix );
    }
    else
    {
        rm->calculateTransitionProbabilities( startAge, endAge,  rate, this->transition_prob_matrix );
    }
