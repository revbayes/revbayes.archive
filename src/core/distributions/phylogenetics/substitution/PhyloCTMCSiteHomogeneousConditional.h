...
namespace RevBayesCore {
    ...
    protected:
        ...
        virtual bool                                        isSitePatternCompatible( std::map<RbBitSetGeneral, size_t> );
...
// Thanks to MH
template<class charType>
bool RevBayesCore::PhyloCTMCSiteHomogeneousConditional<charType>::isSitePatternCompatible( std::map<RbBitSetGeneral, size_t> charCounts )
{
    ...
    for(std::map<RbBitSetGeneral, size_t>::iterator it = charCounts.begin(); it != charCounts.end(); it++)
    {
        const RbBitSetGeneral& r = it->first;
