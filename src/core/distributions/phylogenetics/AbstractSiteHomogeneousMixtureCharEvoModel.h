...
numMixtures( nMix ),
tau( t ), 
transitionProbMatrices( std::vector<TransitionProbabilityMatrix>(numMixtures, TransitionProbabilityMatrix(numChars) ) ),
partialLikelihoods( new double[2*tau->getValue().getNumberOfNodes()*numMixtures*numSites*numChars] ),
...
dirtyNodes( std::vector<bool>(tau->getValue().getNumberOfNodes(), true) ) {
    ...
    activeLikelihoodOffset      =  tau->getValue().getNumberOfNodes()*numMixtures*numPatterns*numChars;
    nodeOffset                  =  numMixtures*numPatterns*numChars;
...
dirtyNodes( n.dirtyNodes ) {
    ...
    memcpy(partialLikelihoods, n.partialLikelihoods, 2*tau->getValue().getNumberOfNodes()*numMixtures*numPatterns*numChars*sizeof(double));
...
void RevBayesCore::AbstractSiteHomogeneousMixtureCharEvoModel<charType, treeType>::computeRootLikelihood( size_t root, size_t left, size_t right) {
    ...
    this->lnProb -= log( numMixtures ) * numSites;
...
void RevBayesCore::AbstractSiteHomogeneousMixtureCharEvoModel<charType, treeType>::setValue(AbstractCharacterData *v) {
    ...
    delete [] partialLikelihoods;
    partialLikelihoods = new double[2*tau->getValue().getNumberOfNodes()*numMixtures*numPatterns*numChars];
