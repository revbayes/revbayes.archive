 ...
 */
GraphFlipChordProposal::GraphFlipChordProposal( StochasticNode<MatrixReal> *n, const RbVector<long>& v, double l, bool s) : Proposal(),
array(NULL),
matrix( n ),
vertices( v ),
 ...
 *
 */
void GraphFlipChordProposal::cleanProposal( void )
{
    ; // do nothing
}
 ...
 */
GraphFlipChordProposal* GraphFlipChordProposal::clone( void ) const
{
    
    return new GraphFlipChordProposal( *this );
}
 ...
 */
const std::string& GraphFlipChordProposal::getProposalName( void ) const
{
 ...
 */
double GraphFlipChordProposal::doProposal( void )
{
    ...
    // sample a node from nodes with 2+ edges
    std::set<size_t> possible_vertices;
...
//
//    size_t index = size_t( rng->uniform01() * vertex_list_length );
//    size_t the_vertex = vertices[index];
 ...
 *
 */
void GraphFlipChordProposal::prepareProposal( void )
{
 ...
 */
void GraphFlipChordProposal::printParameterSummary(std::ostream &o) const
{
 ...
 */
void GraphFlipChordProposal::undoProposal( void )
{
 ...
 */
void GraphFlipChordProposal::swapNodeInternal(DagNode *oldN, DagNode *newN)
{
    if (oldN == array)
    {
 ...
 */
void GraphFlipChordProposal::tune( double rate )
{
