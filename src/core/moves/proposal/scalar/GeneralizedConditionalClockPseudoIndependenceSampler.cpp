 ...
 */
GeneralizedConditionalClockISProposal::GeneralizedConditionalClockISProposal( StochasticNode<double>* n, TypedDagNode<double> *stl, double s, int k, int N, bool l ) : Proposal(),
    variable( n ),
    substitutionTreeLength ( stl ),
    sigma ( s ),
    nInv ( k ),
    nSites ( N ),
    isLogScale ( l ),
    storedValue( 0.0 )
{
    ...
    addNode( variable );
    addNode( substitutionTreeLength );
 ...
 *
 */
void GeneralizedConditionalClockISProposal::cleanProposal( void )
{
 ...
 */
GeneralizedConditionalClockISProposal* GeneralizedConditionalClockISProposal::clone( void ) const
{
    
    return new GeneralizedConditionalClockISProposal( *this );
}
 ...
 */
const std::string& GeneralizedConditionalClockISProposal::getProposalName( void ) const
{
    static std::string name = "GeneralizedConditionalClockIS";
 ...
 */
double GeneralizedConditionalClockISProposal::doProposal( void )
{
...
//    std::cout << "PROPOSING proposal on clock rate" <<std::endl;
    ...
    double logSubsTL = std::log(substitutionTreeLength->getValue());
    ...
    double scaling = std::exp(new_logSubsTL)/(substitutionTreeLength->getValue());
    double newVal;
    if ( isLogScale ) {
        newVal = std::log(val*scaling);
    } else {
        newVal = val*scaling;
    }
 ...
 */
void GeneralizedConditionalClockISProposal::prepareProposal( void )
{
 ...
 */
void GeneralizedConditionalClockISProposal::printParameterSummary(std::ostream &o) const
{
 ...
 */
void GeneralizedConditionalClockISProposal::undoProposal( void )
{
 ...
 */
void GeneralizedConditionalClockISProposal::swapNodeInternal(DagNode *oldN, DagNode *newN)
{
    ...
    else if ( oldN == substitutionTreeLength )
    {
        substitutionTreeLength = static_cast<TypedDagNode<double>* >(newN);
    }
 ...
 */
void GeneralizedConditionalClockISProposal::tune( double rate )
{
