 ...
 */
RootTimeSlideUniformProposal::RootTimeSlideUniformProposal( StochasticNode<Tree> *n, TypedDagNode<double> *o ) : Proposal(),
    variable( n ),
    origin( o )
{
    // tell the base class to add the Root
    addNode( variable );
    addNode( origin );
...
double RootTimeSlideUniformProposal::doProposal( void )
{
    ...
    double child_Age   = root.getChild( 0 ).getAge();
    if ( child_Age < root.getChild( 1 ).getAge())
    {
        child_Age = root.getChild( 1 ).getAge();
    }
 ...
 * the value of the variable/DAG-Root to its original value.
 */
