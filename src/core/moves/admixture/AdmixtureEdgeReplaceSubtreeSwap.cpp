//
//  AdmixtureEdgeReplaceSubtreeSwap.cpp
//  rb_mlandis
//
...
#include "AdmixtureEdgeReplaceSubtreeSwap.h"
...
AdmixtureEdgeReplaceSubtreeSwap::AdmixtureEdgeReplaceSubtreeSwap(StochasticNode<AdmixtureTree> *v, DeterministicNode<std::vector<double> >* res, double d, int ag, int me, bool asa, double w) : MoveOld( v, w), variable( v ), residuals(res), changed(false), failed(false), activeGen(ag), maxEvents(me), allowSisterAdmixture(asa), delta(d) {
...
/* Clone object */
AdmixtureEdgeReplaceSubtreeSwap* AdmixtureEdgeReplaceSubtreeSwap::clone( void ) const {
    
    return new AdmixtureEdgeReplaceSubtreeSwap( *this );
}



const std::string& AdmixtureEdgeReplaceSubtreeSwap::getMoveName( void ) const {
    static std::string name = "AdmixtureEdgeReplaceSubtreeSwap";
    
    return name;
}

void AdmixtureEdgeReplaceSubtreeSwap::findDescendantTips(std::set<AdmixtureNode*>& s, AdmixtureNode* p)
{
...
void AdmixtureEdgeReplaceSubtreeSwap::findNewBrothers(std::vector<AdmixtureNode *> &b, AdmixtureNode &p, AdmixtureNode *n) {
...
void AdmixtureEdgeReplaceSubtreeSwap::storeAdmixtureEventsForLineage(AdmixtureNode* p)
{
...
/** Perform the move */
double AdmixtureEdgeReplaceSubtreeSwap::performSimpleMove( void ) {
    ...
    } while (regraftNode == pruneNode || regraftChild == pruneNode || pruneChild == regraftNode ||
             pruneChild->getAge() >= regraftParent->getAge() || regraftChild->getAge() >= pruneParent->getAge());
    ...
    // perform SPR
    int pruneNphIdx = 0;
    if (pruneChild == &pruneNode->getTopologyChild(pruneNphIdx))
        pruneNphIdx = 1;
    pruneNephew = &pruneNode->getChild(pruneNphIdx);
    pruneParent = &pruneNode->getParent();
    
    int regraftNphIdx = 0;
    if (regraftChild == &regraftNode->getTopologyChild(regraftNphIdx))
        regraftNphIdx = 1;
    regraftNephew = &regraftNode->getChild(regraftNphIdx);
    regraftParent = &regraftNode->getParent();
    ...
    pruneNode->addChild(regraftNephew,false);
    regraftNephew->setParent(pruneNode,false);
    regraftParent->addChild(pruneNode,false);
    
    regraftParent->removeChild(regraftNode,false);
    regraftNode->removeChild(regraftNephew,false);
    regraftNode->addChild(pruneNephew,false);
    pruneNephew->setParent(regraftNode,false);
    pruneParent->addChild(regraftNode,false);
    */
    
    pruneNode->removeChild(pruneChild,false);
    regraftNode->removeChild(regraftChild,false);
    pruneNode->addChild(regraftChild,false);
    pruneChild->setParent(regraftNode,false);
    regraftNode->addChild(pruneChild,false);
...
void AdmixtureEdgeReplaceSubtreeSwap::rejectSimpleMove( void ) {
    ...
    if ( !failed ) {
        ...
        */
        pruneNode->removeChild(regraftChild,false);
        regraftNode->removeChild(pruneChild,false);
        pruneNode->addChild(pruneChild,false);
        pruneChild->setParent(pruneNode,false);
        regraftNode->addChild(regraftChild,false);
        regraftChild->setParent(regraftNode,false);
...
void AdmixtureEdgeReplaceSubtreeSwap::acceptSimpleMove(void) {
...
void AdmixtureEdgeReplaceSubtreeSwap::swapNode(DagNode *oldN, DagNode *newN) {
...
void AdmixtureEdgeReplaceSubtreeSwap::rejectMove( void ) {
    changed = false;
...
void AdmixtureEdgeReplaceSubtreeSwap::acceptMove( void ) {
    // nothing to do
    changed = false;
...
bool AdmixtureEdgeReplaceSubtreeSwap::isActive(unsigned long g) const {
    
    return g > activeGen;
}

double AdmixtureEdgeReplaceSubtreeSwap::performMove( double &probRatio ) {
