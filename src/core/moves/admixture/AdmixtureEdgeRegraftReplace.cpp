//
//  AdmixtureEdgeRegraftReplace.cpp
//  rb_mlandis
//
...
AdmixtureEdgeRegraftReplace::AdmixtureEdgeRegraftReplace(StochasticNode<AdmixtureTree> *v, DeterministicNode<std::vector<double> >* res, double d, int ag, int me, bool asa, double w) : MoveOld( v, w), variable( v ), residuals(res), changed(false), failed(false), activeGen(ag), maxEvents(me), allowSisterAdmixture(asa), delta(d) {
...
/* Clone object */
AdmixtureEdgeRegraftReplace* AdmixtureEdgeRegraftReplace::clone( void ) const {
    
    return new AdmixtureEdgeRegraftReplace( *this );
}



const std::string& AdmixtureEdgeRegraftReplace::getMoveName( void ) const {
    static std::string name = "AdmixtureEdgeRegraftReplace";
    
    return name;
}

void AdmixtureEdgeRegraftReplace::findDescendantTips(std::set<AdmixtureNode*>& s, AdmixtureNode* p)
{
...
/** Perform the move */
double AdmixtureEdgeRegraftReplace::performSimpleMove( void ) {
    ...
    //std::cout << admixtureParents.size() << "\n";
    ...
    else
    {
        ...
        // get random admixture edge
        AdmixtureNode* p = admixtureParents[rng->uniform01()];
        
        // ignore if adm betw sister lineages
        if (&p->getTopologyParent() == &p->getAdmixtureChild().getTopologyParent()
            || p->getAdmixtureChild().getTopologyParent().isRoot()
            || p->getTopologyParent().isRoot()
            || p->isOutgroup() != p->getAdmixtureChild().isOutgroup() )
        {
            failed = true;
            return RbConstants::Double::neginf;
        }
        ...
        // store and clear all admixture edges
        //AdmixtureNode* q_it = &p->getTopologyChild(0).getParent();
        
        if (GLOBAL_RNG->uniform01() < 0.5)
        {
            regraftChild = &p->getTopologyChild(0);
            pruneChild = &p->getAdmixtureChild().getTopologyChild(0);
        }
        else
        {
            pruneChild = &p->getTopologyChild(0);
            regraftChild = &p->getAdmixtureChild().getTopologyChild(0);
        }
        
        regraftChild = &p->getTopologyChild(0);
        pruneChild = &p->getAdmixtureChild().getTopologyChild(0);
        AdmixtureNode* q_it = &pruneChild->getParent();
        ...
        while (q_it->getNumberOfChildren() != 2)
        {
            ...
            //std::cout << &qp->getParent() << " -> " << qp << " -> " << qpc << "\n";
            //std::cout << &qc->getParent() << " -> " << qc << " -> " << qcc << "\n";
            ...
            q_it = &pruneChild->getParent();
            
            numEvents++;
        }
        
        // make divergence topology proposal
        double admixtureAge = p->getAge();
        regraftParent = &regraftChild->getParent();
        while (admixtureAge > regraftParent->getAge())
        {
            regraftChild = regraftParent;
            regraftParent = &regraftParent->getParent();
            //std::cout << admixtureAge << " " << regraftParent->getAge() << "\n";
            
        }
        
        pruneNode = &pruneChild->getParent();
        storedAge = pruneNode->getAge();
        while (admixtureAge > pruneNode->getAge())
        {
            pruneChild = pruneNode;
            pruneNode = &pruneNode->getParent();
            //std::cout << admixtureAge << " " << pruneNode->getAge() << "\n";
        }
        
        pruneParent = &pruneNode->getParent();
        pruneNephew = &pruneNode->getChild(0);
        if (pruneNephew == pruneChild)
            pruneNephew = &pruneNode->getChild(1);
        
        //if (pruneNode->isRoot())
        //    std::cout << "pruneNode->root!\n";
        //if (pruneParent->isRoot())
        //    std::cout << "pruneParent->root!\n";
        //std::cout << pruneParent << " -> " << pruneNode << " -> " << pruneChild << "\n";
        //std::cout << regraftParent << " -> " << regraftChild << "\n";
...
void AdmixtureEdgeRegraftReplace::rejectSimpleMove( void ) {
    ...
    if ( !failed ) {
        ...
        // undo properly...
        //AdmixtureTree& tau = variable->getValue();
        //tau = storedAdmixtureTree;
    }
}

void AdmixtureEdgeRegraftReplace::acceptSimpleMove(void) {
...
void AdmixtureEdgeRegraftReplace::swapNode(DagNode *oldN, DagNode *newN) {
...
void AdmixtureEdgeRegraftReplace::rejectMove( void ) {
    changed = false;
...
void AdmixtureEdgeRegraftReplace::acceptMove( void ) {
    // nothing to do
    changed = false;
...
double AdmixtureEdgeRegraftReplace::performMove( double &probRatio ) {
