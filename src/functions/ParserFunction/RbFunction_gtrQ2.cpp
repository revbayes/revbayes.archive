...
const ArgumentRule RbFunction_gtrQ::argRules[] = {
    ...
    ArgumentRule("pi",      Simplex::dataType),
    ArgumentRule("revrate", Simplex::dataType),
    ArgumentRule()
};
...
RbFunction_gtrQ::RbFunction_gtrQ()
    : RbStandardFxn(), qmatVec(RbDataVector(RateMatrix::dataType)) {
}

/** Copy constructor */
RbFunction_gtrQ::RbFunction_gtrQ(const RbFunction_gtrQ& s)
    : RbStandardFxn(s), qmatVec(RbDataVector(RateMatrix::dataType)) {
}
...
RbDataType* RbFunction_gtrQ::execute(void) {
    ...
    RbDataVector& piVec = arguments[0]->getValue();
    RbDataVector& rVec  = arguments[1]->getValue();
    ...
    for (int i=0; i<dim0; i++) {
        RateMatrix& qmat = static_cast<RateMatrix &> (*(qmatVec[i]));
        Simplex&    pi   = static_cast<Simplex &>    (*(piVec[i%dim1]));
        Simplex&    r    = static_cast<Simplex&>     (*(rVec[i%dim2]));
        ...
        if (rDim != (piDim*piDim - piDim)/2) {
            delete qmatVec[i];
            qmatVec[i] = new RateMatrix();   /* Null rate matrix */
            continue;
        }
        else if (qmat.size() != piDim)
            {
            delete qmatVec[i];
            qmatVec[i] = new RateMatrix(piDim);
