...
const MemberRules& Dist_dtmm::getMemberRules( void ) const {
    ...
    if ( !rulesSet ) {
        memberRules.push_back( new ValueRule( "m", ParserTransitionProbabilityMatrix::getClassTypeSpec()) );
...
//    return *randomValue;
//}
void Dist_dtmm::rvSingleValue( std::vector<size_t> &offset ) {
...
void Dist_dtmm::setInternalParameters(const std::vector<RbValue<void *> > &p) {
    
    transProbabilityMatrix.value        = static_cast<TransitionProbabilityMatrix*>( p[0].value );
    transProbabilityMatrix.lengths      = p[0].lengths;
    
    initialState.value                  = static_cast<char*>( p[1].value );
    initialState.lengths                = p[1].lengths;
    
    randomVariable.value                = static_cast<char*>( p[2].value );
