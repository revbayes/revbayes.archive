...
public:
    ...
    bool                                        isTouched(void) const;                                                                      //!< Is node touched by move or parser?
//    void                                        markChildrenForRecalculation(void);                                                         //!< Mark all my children for recalculation
    void                                        removeParentNode(InferenceDagNode* p);                                                      //!< Remove a child node
    ...
    virtual double                              calculateEliminatedLnProbability(bool force = false) = 0;                                   //!< Calculate summed log conditional probability over all possible states
...
//    virtual void                                makeProbabilitiesDirty(void) = 0;
    virtual void                                makeLikelihoodsDirty(void) = 0;                                                               //!< Tell this node that the likelihoods need to be updated
