...
LookupNode::LookupNode(DAGNode* var, IndexArgs&  indxArgs) :
    DeterministicNode(var->getValueType()), baseVariable(var), baseLookup(NULL), memberName(), indexArgs(indxArgs), valueDim(0) {
...
LookupNode::LookupNode(LookupNode* baseVar, RbString* membrName, IndexArgs&  indxArgs) :
    DeterministicNode(baseVar->getMemberTypeSpec(*membrName).getType()), baseVariable(NULL), baseLookup(baseVar), memberName(membrName), indexArgs(indxArgs), valueDim(0) {
...
LookupNode* LookupNode::cloneDAG(std::map<DAGNode*, DAGNode*>& newNodes) const {
    ...
    copy->value        = copy->lookup()->getValue()->clone();
...
void LookupNode::swapParentNode(DAGNode* oldNode, DAGNode* newNode) {
    ...
    if (oldNode == baseVariable) {
        ...
        baseLookup = dynamic_cast<LookupNode *>( newNode );
    }
    else {
