...
const MemberRules& UniformDistribution::getMemberRules(void) const {
    ...
    if ( !rulesSet ) {
        distUnifMemberRules.push_back( new ArgumentRule( "lower", true, Real::getClassTypeSpec()   , new Real(0.0) ) );
        distUnifMemberRules.push_back( new ArgumentRule( "upper"  , true, Real::getClassTypeSpec(), new RealPos(1.0) ) );
