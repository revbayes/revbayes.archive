#ifndef RevObjectVector_H
#define RevObjectVector_H
...
namespace RevLanguage {
    ...
    /**
     * @brief RevObjectVector: templated vector of Rev object elements
     *
     * RevObjectVector is a vector specialization of WorkspaceContainer. It is templated on the
     * Rev type of the elements of the vector. It is intended for all Rev objects that are neither
     * workspace objects or model objects.
     */
    template <typename rlType>
    class RevObjectVector : public WorkspaceContainer {
        
    public:
        typedef std::vector<rlType*>                vectorRlPtr;
        
        RevObjectVector(void);                                              //!< Default constructor
        RevObjectVector(const vectorRlPtr& v);                              //!< Constructor from rl value pointers
        ...
        virtual RevObjectVector<rlType>*            clone(void) const;                                                  //!< Clone object
 ...
 */
template <typename rlType>
RevObjectVector<rlType>::RevObjectVector( void ) :
WorkspaceContainer()
{
}
 ...
 */
template <typename rlType>
RevObjectVector<rlType>::RevObjectVector( const vectorRlPtr& v ) :
WorkspaceContainer()
{
 ...
 */
template <typename rlType>
rlType& RevObjectVector<rlType>::operator[]( size_t index )
{
 ...
 */
template <typename rlType>
const rlType& RevObjectVector<rlType>::operator[]( size_t index ) const
{
    return static_cast<rlType&>( this->elements[ index ]->getRevObject() ).getValue();
}
...
template <typename rlType>
RevObjectVector<rlType>* RevObjectVector<rlType>::clone() const
{
    return new RevObjectVector<rlType>( *this );
}
 ...
 */
template <typename rlType>
RevPtr<Variable> RevObjectVector<rlType>::findOrCreateElement(const std::vector<size_t>& oneOffsetIndices)
{
...
template <typename rlType>
size_t RevObjectVector<rlType>::getDim(void) const
{
    return 1;
}
 ...
 */
template<typename rlType>
RevPtr<Variable> RevObjectVector<rlType>::getElement( const std::vector<size_t>& oneOffsetIndices )
{
 ...
 */
template<typename rlType>
void RevObjectVector<rlType>::setElements(std::vector<RevObject*> elems, const std::vector<size_t>& lengths)
{
    // Check lengths specification
    if ( lengths.size() != 1 || lengths[0] != elems.size() )
        throw RbException( "Incorrect lengths specification when setting RevObjectVector elements" );
...
template <typename rlType>
const TypeSpec& RevObjectVector<rlType>::getTypeSpec(void) const
{
 ...
 */
template <typename rlType>
void RevObjectVector<rlType>::pop_back( void )
{
    elements.pop_back();
}
 ...
 */
template <typename rlType>
void RevObjectVector<rlType>::pop_front( void )
{
 ...
 */
template <typename rlType>
void RevObjectVector<rlType>::push_back( rlType* x )
{
 ...
 */
template <typename rlType>
void RevObjectVector<rlType>::push_front( rlType* x )
{
 ...
 */
template<typename rlType>
void RevObjectVector<rlType>::printStructure(std::ostream& o) const
{
