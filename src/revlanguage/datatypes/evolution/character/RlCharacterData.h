...
const RevLanguage::TypeSpec& RevLanguage::CharacterData<rlType>::getClassTypeSpec(void) { 
    ...
    static TypeSpec rbClass = TypeSpec( getClassName(), new TypeSpec( RlModelVariableWrapper< rlType >::getClassTypeSpec() ), new TypeSpec( rlType::getClassTypeSpec() ) );
...
const RevLanguage::MethodTable& RevLanguage::CharacterData<rlType>::getMethods(void) const {
    ...
    if ( methodsSet == false ) 
    {
        ...
        methods.setParentTable( &RlModelVariableWrapper< valueType >::getMethods() );
        methodsSet = true;
    }
