...
const RevLanguage::ArgumentRules& RevLanguage::Func__vectorScalarSub<firstValType, secondValType, retType>::getArgumentRules( void ) const
{
    ...
    if ( !rulesSet )
    {
        ...
        argumentRules.push_back( new ArgumentRule( "first" , ModelVector<firstValType>::getClassTypeSpec() , "The left hand side variable.", ArgumentRule::BY_CONSTANT_REFERENCE, ArgumentRule::ANY, NULL ) );
        argumentRules.push_back( new ArgumentRule( "second", secondValType::getClassTypeSpec(), "The right hand side variable.", ArgumentRule::BY_CONSTANT_REFERENCE, ArgumentRule::ANY, NULL ) );
        rulesSet = true;
    }
