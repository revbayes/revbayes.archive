...
RevPtr<RevVariable> Func_ancestralStateTree::execute( void )
{
    ...
    {
        tree = summary.ancestralStateTree(it->getValue(), ancestralstate_traces, burnin, summary_stat, site, joint, verbose);
    }
...
const ArgumentRules& Func_ancestralStateTree::getArgumentRules( void ) const
{
    ...
    if (!rules_set)
    {
        ...
        argumentRules.push_back( new OptionRule( "reconstruction", new RlString("marginal"), reconstruction, "'joint' should only be used to summarize ancestral states sampled from the joint distribution. 'marginal' can be used for states sampled from the joint or marginal distribution." ) );
