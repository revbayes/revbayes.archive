...
const RevLanguage::ArgumentRules& RevLanguage::Func_rep<valType>::getArgumentRules( void ) const
{
    ...
    if ( !rulesSet )
    {
        ...
        argumentRules.push_back( new ArgumentRule( "x", valType::getClassTypeSpec(), "The value that we replicate.", ArgumentRule::BY_VALUE, ArgumentRule::ANY, NULL ) );
        argumentRules.push_back( new ArgumentRule( "n", Natural::getClassTypeSpec(), "How often we replicate the value.", ArgumentRule::BY_VALUE, ArgumentRule::ANY, NULL ) );
        rulesSet = true;
    }
