/*! 
 * \file
 * This file declares the class RbMatrix, a templated class for
 * matrices and common operations on matrices (2D arrays). It
 * also contains definitions for RbMatrix and related functions
 * operating on templated matrices, such as printing and reading.
 *  
 * \brief Declaration and definitions for RbMatrix
 *
 * MrBayes version 4.0 beta
 *
 * (c) Copyright 2005.
 * \version 4.0 Beta
 * \date Last modified: $Date: 2009-09-17 09:47:08 +0200 (Tor, 17 Sep 2009) $
 ...
 * details.
 *
 * $Id: RbMatrix.h 39 2009-09-17 07:47:08Z Hoehna $
 */
...
#pragma mark Template Class Definition

template <class T> 
class RbMatrix { 
 
public: 
                            RbMatrix(void);                                   //!< null constructor (0 X 0 matrix) 
                            RbMatrix(int m, int n);                           //!< creates a m X n matrix without initialization 
                            RbMatrix(int m, int n,  T *a);                    //!< creates a m X n matrix as a view of a one-dimensional array 
                            RbMatrix(int m, int n, const T &a);               //!< creates a m X n matrix initializing all elements to a constant 
                   inline   RbMatrix(const RbMatrix &A);                      //!< creates a m X n matrix with elements shared by another matrix, A 
                            ~RbMatrix(void);                                  //!< destructor 
 
							operator T**() { return &(v[0]); }                //!< type cast operator
							operator const T**() { return &(v[0]); }          //!< type cast operator for const
          inline RbMatrix   &operator=(const T &a);                           //!< assignment operator (all elements have the value a) 
		         RbMatrix   &operator=(const RbMatrix &A) { return ref(A); }  //!< assignment operator (shallow copy, elements share data) 
                     bool   operator==(const RbMatrix &A) const;              //!< equality operator 
		  ...
		  inline RbMatrix   &ref(const RbMatrix &A);                          //!< creates a reference to another matrix (shallow copy) 
                 RbMatrix   copy(void) const;                                 //!< creates a copy of another matrix (deep copy, with separate data elements) 
                 RbMatrix   &inject(const RbMatrix &A);                       //!< copy the values of elements from one matrix to another 
 ...
 * RbMatrix argument. Matrix data are not copied but shared.
 * Thus, in RbMatrix B(A), subsequent changes to A will be
 * reflected by changes in B. For an independent copy, use
 * RbMatrix B(A.copy()), or B = A.copy(), instead. Note
 ...
 */
template <class T>
inline RbMatrix<T>::RbMatrix(const RbMatrix &A) {
 ...
 */
template <class T>
RbMatrix<T>::RbMatrix(int m, int n) {
 ...
 * array will never be destroyed by the RbMatrix class because of the cArray
 ...
 */
template <class T>
RbMatrix<T>::RbMatrix(int m, int n, T *a) {
 ...
 * often used to create a matrix of zeros, as in RbMatrix A(m, n, 0.0).
 * 
 ...
 */
template <class T>
RbMatrix<T>::RbMatrix(int m, int n, const T &a) {
 ...
 */
template <class T>
inline RbMatrix<T>& RbMatrix<T>::ref(const RbMatrix &A) {
 ...
 * data with the copied matrix, e.g. in RbMatrix B(A.copy()).
 *
 * \brief Create independent copy
 * \return Copy of this.
 */
template <class T>
RbMatrix<T> RbMatrix<T>::copy(void) const {
 ...
 * RbMatrix A(n);
 * RbMatrix C(n);
 * RbMatrix B(C);        (elements of B and C are shared) 
 *
 ...
 */
template <class T>
RbMatrix<T> &RbMatrix<T>::inject(const RbMatrix &A) {
...
template <class T>
void RbMatrix<T>::setIdentity(void) {
...
template <class T> std::ostream& operator<<(std::ostream &s, const RbMatrix<T> &A);       //!< operator << 
template <class T> std::istream& operator>>(std::istream &s, RbMatrix<T> &A);             //!< operator >> 
template <class T> RbMatrix<T> operator+(const RbMatrix<T> &A, const RbMatrix<T> &B);     //!< operator + 
template <class T> RbMatrix<T> operator-(const RbMatrix<T> &A, const RbMatrix<T> &B);     //!< operator - 
template <class T> RbMatrix<T> operator*(const RbMatrix<T> &A, const RbMatrix<T> &B);     //!< operator * (matrix multiplication) 
template <class T> RbMatrix<T> &operator+=(const RbMatrix<T> &A, const RbMatrix<T> &B);   //!< operator += 
template <class T> RbMatrix<T> &operator-=(const RbMatrix<T> &A, const RbMatrix<T> &B);   //!< operator -= 
template <class T> RbMatrix<T> &operator*=(const RbMatrix<T> &A, const RbMatrix<T> &B);   //!< operator *= (matrix multiplication)
template <class T> RbMatrix<T> operator+(const T &a, const RbMatrix<T> &B);               //!< operator + for scalar + matrix 
template <class T> RbMatrix<T> operator-(const T &a, const RbMatrix<T> &B);               //!< operator - for scalar - matrix 
template <class T> RbMatrix<T> operator*(const T &a, const RbMatrix<T> &B);               //!< operator * for scalar * matrix 
template <class T> RbMatrix<T> operator/(const T &a, const RbMatrix<T> &B);               //!< operator / for scalar / matrix 
template <class T> RbMatrix<T> operator+(const RbMatrix<T> &A, const T &b);               //!< operator + for matrix + scalar 
template <class T> RbMatrix<T> operator-(const RbMatrix<T> &A, const T &b);               //!< operator - for matrix - scalar 
template <class T> RbMatrix<T> operator*(const RbMatrix<T> &A, const T &b);               //!< operator * for matrix * scalar 
template <class T> RbMatrix<T> operator/(const RbMatrix<T> &A, const T &b);               //!< operator / for matrix / scalar 
template <class T> RbMatrix<T> &operator+=(const RbMatrix<T> &A, const T &b);             //!< operator += for scalar 
template <class T> RbMatrix<T> &operator-=(const RbMatrix<T> &A, const T &b);             //!< operator -= for scalar 
template <class T> RbMatrix<T> &operator*=(const RbMatrix<T> &A, const T &b);             //!< operator *= for scalar 
template <class T> RbMatrix<T> &operator/=(const RbMatrix<T> &A, const T &b);             //!< operator /= for scalar 
template <class T> bool        operator!=(const RbMatrix<T> &A, const RbMatrix<T> &B);    //!< inequality (operator !=) 
 ...
 */
template <class T>
inline RbMatrix<T> &RbMatrix<T>::operator=(const T &a) {
 ...
 */
template <class T>
bool RbMatrix<T>::operator==(const RbMatrix &A) const {
 ...
 */
template <class T>
std::ostream& operator<<(std::ostream &s, const RbMatrix<T> &A) {
 ...
 */
template <class T>
std::istream& operator>>(std::istream &s, RbMatrix<T> &A) {

	A = RbMatrix<T>();	// make sure we return null matrix on failure
	int M, N;
	char c;
	s >> c;
	if (c != '[')
 ...
 */
template <class T>
RbMatrix<T> operator+(const RbMatrix<T> &A, const T &b) {
 ...
 */
template <class T>
RbMatrix<T> operator+(const T &a, const RbMatrix<T> &B) {
 ...
 */
template <class T>
RbMatrix<T> &operator+=(RbMatrix<T> &A, const T &b) {
 ...
 */
template <class T>
RbMatrix<T> operator+(const RbMatrix<T> &A, const RbMatrix<T> &B) {
 ...
 */
template <class T>
RbMatrix<T>&  operator+=(RbMatrix<T> &A, const RbMatrix<T> &B) {
 ...
 */
template <class T>
bool operator!=(const RbMatrix<T> &A, const RbMatrix<T> &B) {
